...
module.exports = defineConfig(({ mode }) => {
  ...
  return {
      ...
      // 自定义插件，确保CSS文件被正确复制到输出目录
      {
        ...
        writeBundle: {
          ...
          handler(options) {
            ...
            // 读取源CSS文件
            const sourceCssPath = path.resolve(workingDir, 'src/webview/index.css');
    ...
    // CSS处理配置
    css: {
      postcss: path.resolve(workingDir, 'postcss.config.cjs'),
      // 禁用CSS模块化
      modules: {
    ...
    build: {
      ...
      rollupOptions: {
        // 指定入口文件
        input: {
          index: path.resolve(workingDir, 'src/webview/index.tsx'),
        },
        // 输出配置
        output: {
          // 入口文件名格式
    ...
    resolve: {
      alias: {
        ...
        // VSCode特定代码的别名
        '@vscode': path.resolve(workingDir, 'src/webview'),
      },
      // 确保import的文件可以省略扩展名
